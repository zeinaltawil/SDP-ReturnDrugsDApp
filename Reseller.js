...
convertToBuffer = async(reader) => {
  ...
  state = { RESellDrugg: true}
  state = { ViewHistory: true}
  render() {
    const { ViewHistory } = this.state;
    const { RESellDrugg } = this.state;
    return (
    <div class="App" >
    ...
    <div class="welcometxt" >
     <b>  Welcome to the Resellers page </b>
    <br/> 
    </div>
       <br/>
       <br/>
      ...
      <div class= "center">
      <button class="barButtons barButtons1" onClick={() => this.setState({ RESellDrugg: !RESellDrugg })}>
                    { RESellDrugg ? 'Cancel' : 'Resell Medicine' }
                </button>
                { RESellDrugg && (
                    <div id="the div you want to show and hide">
                       <br/>
      <br/>
      <b style={{ color: 'rgb(204, 221, 255)' }}> To Resell medicine: 1) Click (Scan QR Code) to enable camera and read the medicine barcode. </b> 
      <br/>
      <br/>
       <b style={{ color: 'rgb(204, 221, 255)' }}> 2) Once the Ethereum address is shown, specify the quantity and resell.  </b> 
       <br/>
       <br/>
      <form onSubmit={this.onScan}>
          <button  class="button" type="submit"> Scan the QR Code</button>
          </form>
       ...
       <br/>
            <button class="button"   type="submit">  Resell Drug </button>
          </form>
